...
//const string strMessageMagic = "RealPointCoin Signed Message:\n";
const string strMessageMagic = "RealPointCoin Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    if (fCheckInputs)
    {
        ...
        if (nFees < txMinFee)
            return error("CTxMemPool::accept() : not enough fees %s, %" PRIszu " < %" PRIszu ,
                         hash.ToString().c_str(),
                         nFees, txMinFee);
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    if (fDebug && GetBoolArg("-printcreation"))
        printf("GetProofOfStakeReward(): create=%s nCoinAge=%" PRIszu "\n", FormatMoney(nSubsidy).c_str(), nCoinAge);
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    printf("InvalidChainFound: invalid block=%s  height=%d  trust=%s  blocktrust=%" PRIszu "  date=%s\n",
      pindexNew->GetBlockHash().ToString().substr(0,20).c_str(), pindexNew->nHeight,
      CBigNum(pindexNew->nChainTrust).ToString().c_str(), nBestInvalidBlockTrust.Get64(),
      DateTimeStrFormat("%x %H:%M:%S", pindexNew->GetBlockTime()).c_str());
    printf("InvalidChainFound:  current best=%s  height=%d  trust=%s  blocktrust=%" PRIszu "  date=%s\n",
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfWork())
    {
        ...
        if (vtx[0].GetValueOut() > nReward)
            return DoS(50, error("ConnectBlock() : coinbase reward exceeded (actual=%" PRIszu " vs calculated=%" PRIszu ")",
                   vtx[0].GetValueOut(),
                   nReward));
    }
    if (IsProofOfStake())
    {
        ...
        if (nStakeReward > nCalculatedStakeReward)
            return DoS(100, error("ConnectBlock() : coinstake pays too much(actual=%" PRIszu " vs calculated=%" PRIszu ")", nStakeReward, nCalculatedStakeReward));
    }
...
bool CTransaction::GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        if (fDebug && GetBoolArg("-printcoinage"))
            printf("coin age nValueIn=%" PRIszu " nTimeDiff=%d bnCentSecond=%s\n", nValueIn, nTime - txPrev.nTime, bnCentSecond.ToString().c_str());
    }
...
bool CBlock::GetCoinAge(uint64_t& nCoinAge) const
{
    ...
    if (nCoinAge == 0) // block coin age minimum 1 coin-day
        nCoinAge = 1;
    if (fDebug && GetBoolArg("-printcoinage"))
        printf("block coin age total nCoinDays=%" PRIszu "\n", nCoinAge);
    return true;
}
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (IsProofOfStake())
    {
        ...
        if (!CheckCoinStakeTimestamp(GetBlockTime(), (int64_t)vtx[1].nTime))
            return DoS(50, error("CheckBlock() : coinstake timestamp violation nTimeBlock=%" PRIszu " nTimeTx=%u", GetBlockTime(), vtx[1].nTime));
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "RealPointCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "The Times 01/Nov/2016 Real Points Coins are ready to start";
        CTransaction txNew;
        txNew.nTime = 1478070277;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1478070277;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 3182566 : 216178;
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x91df4f53e169fcf9a1ab7ad091d1ed43a704de6317ffb0165fa605459ec96f06"));
        block.print();
...
bool LoadExternalBlockFile(FILE* fileIn)
{
    ...
    printf("Loaded %i blocks from external file in %" PRIszu "ms\n", nLoaded, GetTimeMillis() - nStart);
    return nLoaded > 0;
}
...
unsigned char pchMessageStart[4] = { 0x70, 0x35, 0x22, 0x06 };
